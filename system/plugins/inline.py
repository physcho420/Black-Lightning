...
async def detailed(client, query: CallbackQuery):
    ...
    await query.edit_message_text(f"Master Left Reason - {Variable.AFK_PM_MESSAGE}", reply_markup=InlineKeyboardMarkup(m))
...
async def query_hndr(client, message):
        ...
        # if light_pulu_name in  str(COMMAND_HELP[light_pulu_name]):
           ...
           mkp = [
            ...
            [
             InlineKeyboardButton(
                    text="Back", callback_data=f"back_{a}_{type_}"
                )
            ],
        ]
...
async def chill(client, message):
    if message.from_user.id != (await app.get_me()).id :
        await message.answer("Fumk of bimtch")
        return
...
async def ho(client, message):
    ...
    reply_markup = help_menu(o, sv, "help", alive=alive)
...
def help_menu(numa, setv, prefix, alive: bool =  False):
    number_of_rows = 10
    number_of_cols = 2
